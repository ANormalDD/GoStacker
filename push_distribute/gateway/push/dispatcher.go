			...
			for {
				...
				case task, ok := <-PushTaskChan:
					...
					func() {
						...
						if err != nil {
							if err == ErrNoConn {
								// query central server for user's gateway
								gid, gaddr, found, qerr := centerclient.QueryUser(task.UserID)
								if qerr != nil {
									zap.L().Warn("Failed to query central server", zap.Error(qerr))
								}
								if found && gaddr != "" {
									// forward the marshaled message to the gateway address
									fwdErr := centerclient.ForwardToGateway(gaddr, task.MarshaledMsg)
									if fwdErr != nil {
										zap.L().Error("Failed to forward message to gateway", zap.String("gateway_id", gid), zap.String("gateway_addr", gaddr), zap.Error(fwdErr))
										// fallback to offline storage
										_ = redis.RPushWithRetry(2, "offline:push:"+strconv.FormatInt(task.UserID, 10), task.MarshaledMsg)
									}
								} else {
									// no remote gateway found, store offline
									_ = redis.RPushWithRetry(2, "offline:push:"+strconv.FormatInt(task.UserID, 10), task.MarshaledMsg)
								}
							} else {
								// If WebSocket push fails due to other error, fallback to Redis push

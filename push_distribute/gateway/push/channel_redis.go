...
// RedisRPushWait pushes a raw marshaled message into wait queue for user (used by forward handler)
func RedisRPushWait(uidStr string, msgRaw []byte) error {
	// ensure user is in wait set
	if err := redis.SAddWithRetry(2, "wait:push:set", uidStr); err != nil {
		return err
	}
	if err := redis.RPushWithRetry(2, "wait:push:"+uidStr, msgRaw); err != nil {
		return err
	}
	return nil
}
